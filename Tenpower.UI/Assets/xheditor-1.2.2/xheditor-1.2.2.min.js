...
!function () {
    ...
    a.mapLang = {}, a.setLang = function (c, d) {
        ...
        var d = a.mapLang[c];
        if (d === b && (d = c.replace(/{#([^{}]+)}/g, function (b, c) { var d = a.mapLang[c]; return d ? d : c })), "string" == typeof d) {
            var e = arguments; d = d.replace(/{\$(\d+)}/g, function (a, c) { var d = e[parseInt(c, 10) + 1]; return d !== b ? d : a })
        } return d
    }
    ...
    if ($("script[src*=xheditor]").each(function () {
        var a = this.src; return a.match(/xheditor[^\/]*\.js/i) ? (editorRoot = a.replace(/[\?#].*$/, "").replace(/(^|[\/\\])[^\/]*$/, "$1"), !1) : void 0
    }), isIE) {
        try {
            document.execCommand("BackgroundImageCache", !1, !0)
        } catch (e) { } var jqueryVer = $.fn.jquery; jqueryVer && jqueryVer.match(/^1\.[67]/) && ($.attrHooks.width = $.attrHooks.height = null)
    } var specialKeys = {
        27: "esc",
        9: "tab", 32: "space", 13: "enter",
        8: "backspace", 145: "scroll", 20: "capslock",
        144: "numlock", 19: "pause", 45: "insert", 36: "home",
        46: "del", 35: "end", 33: "pageup", 34: "pagedown", 37: "left", 38: "up", 39: "right",
        40: "down", 112: "f1", 113: "f2", 114: "f3", 115: "f4", 116: "f5", 117: "f6", 118: "f7", 119: "f8", 120: "f9", 121: "f10", 122: "f11", 123: "f12"
    ...
    htmlMedia = '<div><label for="xheMediaUrl">{#media.url}</label><input type="text" id="xheMediaUrl" value="http://" class="xheText" /></div><div><label for="xheMediaWidth">{#media.width}</label><input type="text" id="xheMediaWidth" style="width:40px;" value="480" /> <label for="xheMediaHeight">{#media.height}</label><input type="text" id="xheMediaHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="{#dialogOk}" /></div>',
        //为了使用bootstrap样式的表格，屏蔽一些功能 style="display:none"
        htmlTable = '<div><label for="xheTableRows">{#table.rows}</label>' +
        '<input type="text" id="xheTableRows" style="width:40px;" value="3" />'
        + ' <label for="xheTableColumns">{#table.columns}</label>'
        + '<input type="text" id="xheTableColumns" style="width:40px;" value="2" /></div>'
        + '<div style="display:none"><label for="xheTableHeaders">{#table.headers}</label>'
        + '<select id="xheTableHeaders"><option selected="selected" value="">{#none}</option><option value="row">{#table.headersRow}</option><option value="col">{#table.headersCol}</option><option value="both">{#table.headersBoth}</option></select></div>'
        + '<div style="display:none"><label for="xheTableWidth">{#table.width}</label><input type="text" id="xheTableWidth" style="width:40px;" value="200" />'
        + ' <label for="xheTableHeight">{#table.height}</label><input type="text" id="xheTableHeight" style="width:40px;" value="" /></div>'
        + '<div style="display:none"><label for="xheTableBorder">{#table.border}</label><input type="text" id="xheTableBorder" style="width:40px;" value="1" /></div>'
        + '<div style="display:none"><label for="xheTableCellSpacing">{#table.cellSpacing}</label><input type="text" id="xheTableCellSpacing" style="width:40px;" value="1" /> '
        + '<label for="xheTableCellPadding">{#table.cellPadding}</label><input type="text" id="xheTableCellPadding" style="width:40px;" value="1" /></div>'
        + '<div style="display:none"><label for="xheTableAlign">{#table.align}</label><select id="xheTableAlign"><option selected="selected" value="">{#default}</option><option value="left">{#table.alignLeft}</option><option value="center">{#table.alignCenter}</option><option value="right">{#table.alignRight}</option></select></div>'
        + '<div><label for="xheTableCaption">{#table.caption}</label><input type="text" id="xheTableCaption" /></div>'
        + '<div style="text-align:right;"><input type="button" id="xheSave" value="{#dialogOk}" /></div>',
    ...
    var arrDbClick = { a: "Link", img: "Img", embed: "Embed" }, uploadInputname = "filedata", arrEntities = { "<": "&lt;", ">": "&gt;", '"': "&quot;", "®": "&reg;", "©": "&copy;" }, regEntities = /[<>"®©]/g, Xheditor = function (textarea, options) {
        ...
        } settings.tools.match(/(^|,)\s*About\s*(,|$)/i) || (settings.tools += ",About"), settings.tools = settings.tools.split(","), settings.editorRoot && (editorRoot = settings.editorRoot), bAir === !1 && (editorRoot = getLocalUrl(editorRoot, "abs")), settings.urlBase && (settings.urlBase = getLocalUrl(settings.urlBase, "abs")); var idCSS = "xheCSS_" + settings.skin, idContainer = "xhe" + xCount + "_container", idTools = "xhe" + xCount + "_Tool", idIframeArea = "xhe" + xCount + "_iframearea", idIframe = "xhe" + xCount + "_iframe", idFixFFCursor = "xhe" + xCount + "_fixffcursor", headHTML = "", bodyClass = "", skinPath = editorRoot + "xheditor_skin/" + settings.skin + "/", arrEmots = itemEmots, urlType = settings.urlType, urlBase = settings.urlBase, emotPath = settings.emotPath, emotPath = emotPath ? emotPath : editorRoot + "xheditor_emot/", selEmotGroup = ""; arrEmots = $.extend({}, arrEmots, settings.emots), emotPath = getLocalUrl(emotPath, "rel", urlBase ? urlBase : null), bShowBlocktag = settings.showBlocktag, bShowBlocktag && (bodyClass += " showBlocktag"); var arrShortCuts = [];
        this.init = function () {
            function a(a) {
                var b, c = $(a.target);
            ...
            function c(a, b, c, d, e, g) { var h, i, j, k, l = ""; if (h = d.match(/font-family\s*:\s*([^;"]+)/i), h && (l += ' face="' + h[1] + '"'), i = d.match(/font-size\s*:\s*([^;"]+)/i)) { i = i[1].toLowerCase(); for (var m = 0; m < f.length; m++) if (i === f[m].n || i === f[m].s) { j = m + 1; break } j && (l += ' size="' + j + '"', d = d.replace(/(^|;)(\s*font-size\s*:\s*[^;"]+;?)+/gi, "$1")) } if (k = d.match(/(?:^|[\s;])color\s*:\s*([^;"]+)/i)) { var n; if (n = k[1].match(/\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i)) { k[1] = "#"; for (var o = 1; 3 >= o; o++) k[1] += ("0" + (n[o] - 0).toString(16)).slice(-2) } k[1] = k[1].replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i, "#$1$1$2$2$3$3"), l += ' color="' + k[1] + '"' } return d = d.replace(/(^|;)(\s*(font-family|color)\s*:\s*[^;"]+;?)+/gi, "$1"), "" !== l ? (d && (l += ' style="' + d + '"'), "<font" + (c ? c : "") + l + (e ? e : "") + ">" + g + "</font>") : a } function d(a, b, c, d, e) { for (var f, h, i = (c ? c : "") + (d ? d : ""), j = [], k = [], l = 0; l < g.length; l++) f = g[l].r, h = g[l].t, i = i.replace(f, function () { return j.push("<" + h + ">"), k.push("</" + h + ">"), "" }); return i = i.replace(/\s+style\s*=\s*"\s*"/i, ""), (i ? "<span" + i + ">" : "") + j.join("") + e + k.join("") + (i ? "</span>" : "") } var e = ' class="Apple-style-span"', f = settings.listFontsize; if ("write" === b) a = a.replace(/(<(\/?)(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*((\/?)>)/g, function (a, b, c, d, f, g, h) { d = d.toLowerCase(), isMozilla ? "strong" === d ? d = "b" : "em" === d && (d = "i") : isWebkit && ("strong" === d ? (d = "span", c || (f += e + ' style="font-weight: bold;"')) : "em" === d ? (d = "span", c || (f += e + ' style="font-style: italic;"')) : "u" === d ? (d = "span", c || (f += e + ' style="text-decoration: underline;"')) : "strike" === d && (d = "span", c || (f += e + ' style="text-decoration: line-through;"'))); var i, j = ""; if ("del" === d) d = "strike"; else if ("img" === d) f = f.replace(/\s+emot\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i, function (a, b) { return i = b.match(/^(["']?)(.*)\1/)[2], i = i.split(","), i[1] || (i[1] = i[0], i[0] = ""), "default" === i[0] && (i[0] = ""), settings.emotMark ? a : "" }); else if ("a" === d) !f.match(/ href=[^ ]/i) && f.match(/ name=[^ ]/i) && (j += " xhe-anchor"), h && (g = "></a>"); else if ("table" === d && !c) { var k = f.match(/\s+border\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i); (!k || k[1].match(/^(["']?)\s*0\s*\1$/)) && (j += " xhe-border") } var l; if (f = f.replace(/\s+([\w\-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g, function (a, b, c) { return b = b.toLowerCase(), c = c.match(/^(["']?)(.*)\1/)[2], aft = "", isIE && b.match(/^(disabled|checked|readonly|selected)$/) && c.match(/^(false|0)$/i) ? "" : "img" === d && i && "src" === b ? "" : (b.match(/^(src|href)$/) && (aft = " _xhe_" + b + '="' + c + '"', urlBase && (c = getLocalUrl(c, "abs", urlBase))), j && "class" === b && (c += " " + j, j = ""), isWebkit && "style" === b && "span" === d && c.match(/(^|;)\s*(font-family|font-size|color|background-color)\s*:\s*[^;]+\s*(;|$)/i) && (l = !0), " " + b + '="' + c + '"' + aft) }), i) { var m = emotPath + (i[0] ? i[0] : "default") + "/" + i[1] + ".gif"; f += ' src="' + m + '" _xhe_src="' + m + '"' } return l && (f += e), j && (f += ' class="' + j + '"'), "<" + c + d + f + g }), isIE && (a = a.replace(/&apos;/gi, "&#39;")), isWebkit || (a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/gi, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/gi, c)), a = a.replace(/<(td|th)(\s+[^>]*?)?>(\s|&nbsp;)*<\/\1>/gi, "<$1$2>" + (isIE ? "" : "<br />") + "</$1>"); else {
                if (isWebkit) for (var g = [{ r: /font-weight\s*:\s*bold;?/gi, t: "strong" }, { r: /font-style\s*:\s*italic;?/gi, t: "em" }, { r: /text-decoration\s*:\s*underline;?/gi, t: "u" }, { r: /text-decoration\s*:\s*line-through;?/gi, t: "strike" }], h = 0; 2 > h; h++) a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, d),
            ...
            listFontsize: [{ n: "x-small", s: "10px" }, { n: "small", s: "12px" }, { n: "medium", s: "16px" }, { n: "large", s: "18px" }, { n: "x-large", s: "24px" },
                { n: "xx-large", s: "32px" }, { n: "-webkit-xxx-large", s: "48px" }]
            ...
            return this.each(function () {
                if ($.nodeName(this, "TEXTAREA")) if (options === !1) this.xheditor && (this.xheditor.remove(), this.xheditor = null);
                    ...
                    } var editor = new Xheditor(this, options); editor.init() ? (this.xheditor = editor, arrSuccess.push(editor)) : editor = null
                }
            }), 0 === arrSuccess.length && (arrSuccess = !1), 1 === arrSuccess.length && (arrSuccess = arrSuccess[0]), arrSuccess
        }, $.fn.oldVal = $.fn.val, $.fn.val = function (a) { var b, c = this; return a === undefined ? c[0] && (b = c[0].xheditor) ? b.getSource() : c.oldVal() : c.each(function () { (b = this.xheditor) ? b.setSource(a) : c.oldVal(a) }) }, $(function () { $("textarea").each(function () { var a = $(this), b = a.attr("class"); b && (b = b.match(/(?:^|\s)xheditor(?:\-(m?full|simple|mini))?(?:\s|$)/i)) && a.xheditor(b[1] ? { tools: b[1] } : null) }) })
}(XHEDITOR, jQuery);